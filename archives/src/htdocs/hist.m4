m4_include([htdocs/nav.m4])
ppNavHyper()
ppSep(std)
<h1>History of Exponentiation</h1>
<p>
The story of ppIt(tetration) began with ppIt(exponentiation).
</p>
<p>
Just shy of a score after the first records of the "+" and "&minus;" 
notation appeared in 1526, in 1544
the German mathematician ppEm(Michael Stifel) published
a detailed investigation of exponentiation in <i>Arithmetica integra</i>.
Stifel discovered ppIt(logarithms) before John Napier, and gave
an early form of logarithm tables. Stifel also coined the term
ppIt(exponent), also published in <i>Arithmetica integra</i>, 
but it did not find its way into the English language until 1704, 
when the term ppIt(exponential) appeared in
the <i>Lexicon Technicum</i> by John Harris.
</p>
<p>
The Scottish mathematician ppEm(John Napier) is most remembered as the inventor
of ppIt(logarithms), although the idea had been around before him, he was the 
first to use the term, although it did not appear in English until 1615. Napier 
published his major work, <i>Mirifici Logarithmorum Canonis Descriptio</i>, 
in 1614, which contains several dozen pages of logarithm tables, along with a 
lengthy explanation of both logarithms and trigonometry. Napier's work made a 
huge impact on natural sciences across the board.
</p>
<p>
One of the most generous contributors to the development of exponentiation was
ppEm(Leonhard Euler) (pronounced <i>oiler</i>). Euler coined the term ppIt(function), and introduced 
the notation for trigonometric functions we still use today. Exponentiation had
already been defined over the complex numbers, for some time, when he rediscovered in 1748 what is now known as Euler's Formula:
LS(
LI(Euler's Formula:, 
    [[ppE()ppSup(IU()ppV(x)) = cos(ppV(x)) + IU() sin(ppV(x))]])
)TAB()
It was later found, that Euler was not the first to discover the formula that 
now bears his name. It was first proved, in an obscure form, by ppEm(Roger Cotes) 
in 1714. Cotes is also known for working with Isaac Newton during the 
publication of his major work, <i>Philosophiae Naturalis Principia Mathematica</i>, commonly referred to as "<i>The Principia</i>."
</p>
ppSep(std)
<h1>History of Tetration</h1>
<p>
The ppIt(product-logarithm), also known as the Lambert-ppV(W) function, is 
indirectly related to tetration because it is related to the
ppIt(infinite iterated exponential) function. (More detail can be found
in <a href="ident.html">Identities</a>.) 
The ppIt(product-logarithm) was developed by French
mathematician ppEm(Johann Heinrich Lambert) who published
it in his first book in 1758. 
The infinite iterated exponential function is probably the first use of
tetration, although the terminology has varied over time. It has
also been called the ppIt(hyperpower) function, or simply referred to
by its definition: "ppV(x)^ppV(x)^ppV(x)^...". It was not until 1989 that
ppEm(Joseph F. MacDonnell) used the term ppIt(hyperpower) for this function.
In 1778, Euler was the first to mention this function when he proved that 
it converged on the interval ppE()ppSup(&minus;ppE()) &le; ppV(x) &le; ppE()ppSup(1/ppE()).
Another person to write about the infinite iterated exponential
function was Eisenstein in 1844. Later, in 1925, 
P&oacute;lya and Szeg&ouml;
first used the "ppV(W)(ppV(x))" notation that is commonly used today.
</p>
<p>
The notation ppSup(ppV(y))ppV(x) is what most people use for tetration,
and was introduced by ppEm(Hans Maurer) in 1901. Much later, in 1995,
ppEm(Rudy Rucker) published a book called <i>Infinity and the Mind</i>
in which he used this notation, and gave it so much publicity 
that Rucker is sometimes credited with its first use.
</p>
<p>
The ppIt(Ackermann) function is a topic of some confusion, 
because there are at least two versions of
the function. One version has 2 arguments, and is closely
related to ppIt(hyper-operations). Another version
has 3 arguments, and is <i>equivalent</i> to hyper-operations. 
Both of these functions are named
after ppEm(Wilhelm Ackermann) who defined them in 1928. In 1953,
ppEm(Andrzej Grzegorczyk) also published a paper on the 
hyper-operations, hence the sequence of hyper-operations are
sometimes referred to as the ppIt(Grzegorczyk hierarchy).
</p>
<p>
In 1947, ppEm(Reuben Louis Goodstein) coined the term ppIt(tetration), 
and the names for other hyper-operations:
ppGloss(tetration) for hyper4,
ppGloss(pentation) for hyper5,
ppGloss(hexation) for hyper6,
and so on, from the Greek number prefixes for the number that represents
the hyper-operator. It is said that the second half: <i>-ation</i>,
is short for ppIt(iteration), but Goodstein himself gave no such indication.
</p>
<p>
The most modern analysis of tetration is <i>Exponentials reiterated</i>
by ppEm(Robert Arthur Knoebel) published in 1981 in
<i>American Mathematical Monthly</i>, a periodical also host to other
tetration-related articles such as <i>Infinite exponentials</i> published
in 1936 by Barrow about the infinite iterated exponential function.
The most recent analysis of tetration is a website by
ppEm(Ioannis N. Galidakis) called <a href="http://users.forthnet.gr/ath/jgal/math/">Extreme Mathematics</a>. In it, Galidakis gives two extensions
of tetration to real numbers, and an extension of the product-logarithm
to quaternions.
</p>
<p>
In 1976 ppEm(Donald [[E]]. Knuth) invented a notation for hyper-operations
called ppIt(Up-arrow notation). It has been very successful
on the internet, because it allows tetration to be represented
as "<code>x^^y</code>".
More recently, in 1996, ppEm(John Horton Conway) and ppEm(Richard K. Guy)
published a book called <i>The Book of Numbers</i> in which they
put forth yet another hyper-operation notation. Their notation,
called ppIt(Chained-arrow notation) is much more expressive that Knuth's,
and allows operations well beyond the hyper-operations. Another
notation that goes beyond the hyper-operations yet still includes
them was developed by ppEm(Jonathan Bowers) in 2002 called ppIt(Array notation).
Bowers' notation includes an infix notation he calls 
ppIt(extended operator notation) which is equivalent in all respects
to the current <i>de facto</i> standard notation for hyper-operations,
 prominently used in ppEm(Robert Munafo)'s website
about <a href="http://home.earthlink.net/~mrob/pub/math/largenum.html">Large Numbers</a>.
</p>


<!--
 1758 (Johann Heinrich Lambert)
 1778 Euler proved ppV(x)^^0 converged on [.06..1.4]
 1844 Eisenstein on ppV(x)^^00
1889 "hypercomplex" coined
 1901 (Hans Maurer) uses ^{y}x notation

 1925 "W" symbol used by P—lya and Szegš
 1928 Ackermann function
 1936 Barrow "Infinite exponentials" AMM
 1947 "tetration" coined
 1953 (Andrzej Grzegorczyk) published
 1976 (Dr. Donald Knuth) arrow notation
 1981 (Dr. Robert Arthur Knoebel) "Exponentials reiterated" AMM
    http://genealogy.math.ndsu.nodak.edu/html/id.phtml?id=31728
 1982 i was born
 1989 "hyperpower" coined by (Joseph F. MacDonnell)
 1995 (Rudy Rucker) "Infinity and the Mind"
1996 (John Horton Conway) (Richard K. Guy)
2000 Banwell and Jayakumar "showed that a W-function describes the relation between voltage, current and resistance in a diode"
2004 Packel and Yuen "applied the W-function to a ballistic projectile in the presence of air resistance"
2005 Hayes "Other applications have been discovered in statistical mechanics, quantum chemistry, combinatorics, enzyme kinetics, the physiology of vision, the engineering of thin films, hydrology, and the analysis of algorithms"

(Dr. Gerald Arthur Edgar)
    http://www.genealogy.math.ndsu.nodak.edu/html/id.phtml?id=11345
-->
