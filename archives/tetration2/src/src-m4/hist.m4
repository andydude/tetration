HTMLBEGIN(Home of Tetration - History)
SITENAV()
<hr/>
<div class="darkness">
<h1>History of Exponentiation</h1>
<br/>TAB()
The story of EM(tetration) began with EM(exponentiation).
<br/>
<br/>TAB()
Just shy of a score after the first records of the "+" and "&minus;" 
notation appeared in 1526, in 1544
the German mathematician BIG(Michael Stifel) published
a detailed investigation of exponentiation in <i>Arithmetica integra</i>.
Stifel discovered EM(logarithms) before John Napier, and gave
an early form of logarithm tables. Stifel also coined the term
EM(exponent), also published in <i>Arithmetica integra</i>, 
but it did not find its way into the English language until 1704, 
when the term EM(exponential) appeared in
the <i>Lexicon Technicum</i> by John Harris.
<br/>TAB()
The Scottish mathematician BIG(John Napier) is most remembered as the inventor
of EM(logarithms), although the idea had been around before him, he was the 
...
BIG(Leonhard Euler) (pronounced <i>oiler</i>). Euler coined the term EM(function), and introduced 
...
LS(
LI(Euler's Formula:, 
    [[E()SUP(IU()V(x)) = cos(V(x)) + IU() sin(V(x))]])
)TAB()
It was later found, that Euler was not the first to discover the formula that 
now bears his name. It was first proved, in an obscure form, by BIG(Roger Cotes) 
...
The EM(product-logarithm), also known as the Lambert-V(W) function, is 
indirectly related to tetration because it is related to the
EM(infinite iterated exponential) function. (More detail can be found
in <a href="ident.html">Identities</a>.) 
The EM(product-logarithm) was developed by French
mathematician BIG(Johann Heinrich Lambert) who published
...
also been called the EM(hyperpower) function, or simply referred to
by its definition: "V(x)^V(x)^V(x)^...". It was not until 1989 that
BIG(Joseph F. MacDonnell) used the term EM(hyperpower) for this function.
In 1778, Euler was the first to mention this function when he proved that 
it converged on the interval [E()SUP(&minus;E()), E()SUP(1/E())].
...
first used the "V(W)(V(x))" notation that is commonly used today.
<br/>TAB()
The notation SUP(V(y))V(x) is what most people use for tetration,
and was introduced by BIG(Hans Maurer) in 1901. Much later, in 1995,
BIG(Rudy Rucker) published a book called <i>Infinity and the Mind</i>
in which he used this notation, and gave it so much publicity 
that Rucker is sometimes credited with its first use.
<br/>TAB()
The EM(Ackermann) function is a topic of some confusion, 
because there are at least two versions of
the function. One version has 2 arguments, and is closely
related to EM(hyper-operations). Another version
has 3 arguments, and is <i>equivalent</i> to hyper-operations. 
Both of these functions are named
after BIG(Wilhelm Ackermann) who defined them in 1928. In 1953,
BIG(Andrzej Grzegorczyk) also published a paper on the 
hyper-operations, hence the sequence of hyper-operations are
sometimes referred to as the EM(Grzegorczyk hierarchy).
<br/>TAB()
In 1947, BIG(Reuben Louis Goodstein) coined the term EM(tetration), 
and the names for other hyper-operations:
LS(
LI(Tetration, for hyper4)
LI(Pentation, for hyper5)
LI(Hexation, for hyper6)
...
is short for EM(iteration), but Goodstein himself gave no such indication.
<br/>TAB()
The most modern analysis of tetration is <i>Exponentials reiterated</i>
by BIG(Robert Arthur Knoebel) published in 1981 in
...
BIG(Ioannis N. Galidakis) called <a href="http://users.forthnet.gr/ath/jgal/math/">Extreme Mathematics</a>. In it, Galidakis gives two extensions
of tetration to real numbers, and an extension of the product-logarithm
to quaternions.
<br/>TAB()
In 1976 BIG(Donald [[[[E]]]]. Knuth) invented a notation for hyper-operations
called EM(Up-arrow notation). It has been very successful
on the internet, because it allows tetration to be represented
as "<code>x^^y</code>".
More recently, in 1996, BIG(John Horton Conway) and BIG(Richard K. Guy)
...
called EM(Chained-arrow notation) is much more expressive that Knuth's,
...
them was developed by BIG(Jonathan Bowers) in 2002 called EM(Array notation).
Bowers' notation includes an infix notation he calls 
EM(extended operator notation) which is equivalent in all respects
to the current <i>de facto</i> standard notation for hyper-operations,
 prominently used in BIG(Robert Munafo)'s website
about <a href="http://home.earthlink.net/~mrob/pub/math/largenum.html">Large Numbers</a>.


<!--
 1758 (Johann Heinrich Lambert)
 1778 Euler proved V(x)^^0 converged on [.06..1.4]
 1844 Eisenstein on V(x)^^00
